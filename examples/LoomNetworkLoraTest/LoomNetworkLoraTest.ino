...
using NetType = LoomNet::Network<LoomNet::LoraRadio, 6, 6, 12>;
...
NetType network(make_network(CONFIG, NAME), LoomNet::LoraRadio(A2, A4, A0));
...
void loop() {
  ...
  do {
    ...
    if (status & NetStatus::NET_RECV_RDY) {
      ...
      Serial.println(reinterpret_cast<const char*>(frag.get_payload()));
      Serial.print("From: ");
